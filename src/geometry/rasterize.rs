...
use crate::{
    ...
    pipeline::{
        ...
        RenderTarget, RenderTargetDescriptor, Sampler, SamplerObject,
    },
};
...
pub struct AttribInterpolationPass {
    inner: ComputePass<AttribInterpolationCtxWrapper, 1, 2>,
...
impl AttribInterpolationPass {
    ...
    pub fn new(
        ...
        let in_raster = Sampler::wrap(in_raster, ATTRIB_INTERP_SAMPLER_UNIT_RASTER);
    ...
    pub const fn inner(&self) -> &ComputePass<AttribInterpolationCtxWrapper, 1, 2> {
        &self.inner
    }

    pub const fn inner_mut(&mut self) -> &mut ComputePass<AttribInterpolationCtxWrapper, 1, 2> {
        &mut self.inner
    }

    pub fn input_raster(&self) -> &Sampler {
        self.inner.sampler(0)
    }
